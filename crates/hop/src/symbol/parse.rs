...
use super::{context_error::CtxError, Descriptor, GlobalSymbol, Package, Scheme, Symbol};

pub fn parse_symbol(input: &str) -> Result<Symbol<'_>, String> {
...
fn parse_symbol_inner(input: &str) -> PResult<'_, Symbol<'_>> {
    let (input, symbol) = alt((parse_local_symbol, parse_global_symbol))(input)?;
    eof(input)?;
    Ok((input, symbol))
}
...
fn parse_global_symbol(input: &str) -> PResult<'_, Symbol<'_>> {
